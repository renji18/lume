...
const ExpenseList = ({currentDate, setShowCalendar,}: {
  currentDate: Date
  ...
  useEffect(() => {
    const fetchDataForDate = async () => {
      ...
      return getTransactionsForDate(currentDate.toISOString().split('T')[0], user.id)
    }
  ...
  return (
    <View className={`h-full w-screen items-center`}>
      ...
      >
        <Text className="font-gb text-dark_slate text-4xl">{uiDateFormatter(currentDate)}</Text>
      </TouchableOpacity>
      <Text
